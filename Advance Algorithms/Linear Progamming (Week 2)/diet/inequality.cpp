...
Position select_pivot(const Matrix& a, std::vector<bool>& used_rows, std::vector<bool>& used_columns)
{
    ...
    for (int i=pivot_element.row,siz=a.size();i<siz;i++) {
        if (fabs(ma) -fabs(a[i][pivot_element.column]) < EPS) {
...
{
    const long double div = a[pivot_element.row][pivot_element.column];
...
vector<Column> solve_all_equations(int m, const Matrix& A, const vector<long double>& b)
{
    ...
    for (const auto& sub : subsets) {
        ...
        auto sol = solve_equation(eq);

        if (sol.first == 0 && !sol.second.empty()) {
            solutions.emplace_back(std::move(sol.second));
        }
    }
