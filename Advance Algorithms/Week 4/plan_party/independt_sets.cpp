#include <iostream>
#include <vector>

struct vertex {
    int weight;
    std::vector<int> children;
};

using graph = std::vector<vertex>;
using sum = std::vector<int>;

graph read_tree() noexcept
{
    std::ios::sync_with_stdio(false);

    int vertices_count;
    std::cin >> vertices_count;

    graph tree(vertices_count);

    for (int i = 0; i < vertices_count; ++i) {
        std::cin >> tree[i].weight;
    }

    for (int i = 1, from, to; i < vertices_count; ++i) {
        std::cin >> from >> to;
        tree[from - 1].children.emplace_back(to - 1);
        tree[to - 1].children.emplace_back(from - 1);
    }

    return tree;
}

int comp(const graph& tree, const int vertex, sum& s, const int parent) noexcept
{
    if (s[vertex] == -1) {
        if (tree[vertex].children.empty()) {
            s[vertex] = tree[vertex].weight;
        }
        else {
            auto m1 = tree[vertex].weight;
            for (auto u : tree[vertex].children) {
                if (u == parent)
                    continue;
                for (auto w : tree[u].children) {
                    if (w != vertex) {
                        m1 += comp(tree, w, s, u);
                    }
                }
            }
            auto m0 = 0;
            for (auto u : tree[vertex].children) {
                if (u != parent) {
                    m0 += comp(tree, u, s, vertex);
                }
            }

            s[vertex] = std::max(m1, m0);
        }
    }

    return s[vertex];
}

int independent_sets(const graph& tree) noexcept
{
    size_t size = tree.size();
    if (size == 0)
        return 0;

    auto s = sum(size, -1);

    return comp(tree, 0, s, -1);
}

int main()
{
    const graph tree = read_tree();
    std::cout << independent_sets(tree) << std::endl;
    return 0;
}
